...
def create_gt_final_datasets(datasets, cl_graphs, drug_mat, args):
    """
    Create Pytorch datasets.
    Note that train should contain train and validation, since the hyperparameters should have been trained already.
    """
    logging.info("Called create_gt_final_datasets()")
...
def create_gt_loaders(datasets, cl_graphs, drug_mat, args):
...
#                 ic50 = ic50.to(self.device)
    ...
    def optimize(params, 
                 kfold_splits: int=5):
        """
        Perform bayesian hyperparametertuning.
        """
        # Extract the hyperparameters from the params list
        learning_rate, batch_size, weight_decay = params
        ...
        for train_index, val_index in kf.split(X):
            ...
            model = train_model(X_train, y_train, weight_decay=weight_decay, batch_size=batch_size, learning_rate=learning_rate)
