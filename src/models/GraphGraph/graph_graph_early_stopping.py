...
#                             nn.Dropout(p=dropout),
                    ...
                    self.cell_emb = Sequential('x, edge_index, batch', 
                        [
                            (GATConv(in_channels=4, out_channels=8), 'x, edge_index -> x1'),
                            nn.BatchNorm1d(8),
                            nn.ReLU(inplace=True),
                            (GATConv(in_channels=8, out_channels=8), 'x1, edge_index -> x2'),
                            nn.BatchNorm1d(8),
                            nn.ReLU(inplace=True),  
                            (GATConv(in_channels=8, out_channels=8), 'x2, edge_index -> x3'),
                            nn.BatchNorm1d(8),
#                             nn.ReLU(inplace=True),                
                            (global_max_pool, 'x3, batch -> x4'), 
                            nn.Linear(8, 512),
